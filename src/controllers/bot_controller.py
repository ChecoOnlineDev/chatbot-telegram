...
from src.view.bot_view import CommonBotView, ConsultServiceBotView, NavigationMenuBotView
...
class BotController:
    ...
    def handle_message(self, dto: HandleMessageDto) -> ButtonsDto:
        ...
        state = self.user_states.get(dto.user_id, BotState.START)

        # 1. Lógica Global: Botón 'Volver' siempre disponible
        if text == MainMenuOptions.VOLER.value:
            self.user_states[dto.user_id] = BotState.MAIN_MENU
            return self.common_bot_view.welcome_message()

        # 2. Dispatcher: Delegación por estado
        if state == BotState.START:
            return self._handle_start(dto)
        
        elif state == BotState.MAIN_MENU:
            return self._handle_main_menu(dto)
        
        elif state == BotState.WAITING_FOR_FOLIO:
            return self._handle_waiting_for_folio(dto)
